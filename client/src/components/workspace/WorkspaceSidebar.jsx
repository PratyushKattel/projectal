...
const WorkspaceSidebar = ({ onInviteClick }) => {
  ...
  return (
    <div className="w-64 bg-surface shadow-lg p-6 flex flex-col min-h-screen border-r border-gray-200">
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-primary font-poppins truncate" title={currentWorkspace?.name}>
          {currentWorkspace?.name || "Workspace"}
        </h2>
        {currentWorkspace?.role && (
          <div className={`mt-2 inline-flex items-center px-2 py-0.5 rounded text-xs font-medium border ${getRoleBadgeColor(currentWorkspace.role)}`}>
            {currentWorkspace.role}
          </div>
        )}
      </div>
